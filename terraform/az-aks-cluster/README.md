...
This solution uses a Terraform template to launch a fully configured Azure Kubernetes Service (AKS) managed Kubernetes (k8s) cluster in an existing Resource Group and VNET. By default it expects 10.112.0.0/12 for the VNET CIDR and 10.127.254.0/24, 10.127.254.252.0/24 and 10.127.253.0/24 to be assigned to the mgmt, external and internal subnets respectively.
...
| <a name="input_bigIqPassword"></a> [bigIqPassword](#input\_bigIqPassword) | Admin Password for BIG-IQ | `string` | `"Canada12345!"` | no |
...
| <a name="input_f5_password"></a> [f5\_password](#input\_f5\_password) | BIG-IP Password or Key Vault secret name (value should be Key Vault secret name when az\_key\_vault\_authentication = true, ex. my-bigip-secret) | `string` | `"Canada12345!"` | no |
